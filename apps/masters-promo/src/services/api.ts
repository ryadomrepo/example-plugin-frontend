...
export class CharmDirectApiService {
   ...
   */
  static async getStaffMedia(params: GetStaffMediaParams): Promise<StaffMediaResponse> {
    try {
      const url = new URL(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.getStaffMedia}`);
      ...
      LoggerUtil.error(`Ошибка при загрузке данных для мастера ${params.staff_id}:`, error);
      throw error;
    }
  }
   ...
   */
  static async getMultipleStaffMedia(staffIds: string[]): Promise<StaffMediaResponse[]> {
    try {
      ...
      if (errors.length > 0) {
        LoggerUtil.warn('Ошибки при загрузке данных некоторых мастеров:', errors);
      }
      
      LoggerUtil.info(`Успешно загружены данные для ${successfulResults.length} из ${staffIds.length} мастеров`);
