...
interface SlotConfig {
  ...
  componentOptions?: { html?: string };
  containerType?: string;
  containerOptions?: { masterId?: number };
}

interface MockWidgetApi {
  addSlotInfo: (config: SlotConfig) => string;
}

// Mock реализация Widget API
const mockWidgetApi: MockWidgetApi = {
  addSlotInfo: (config: SlotConfig) => {
    ...
    if (config.componentType === 'html' && config.componentOptions?.html) {
      ...
      if (element) {
        ...
        LoggerUtil.info(
          `Mock Widget API: HTML элемент добавлен в DOM`,
          element.id,
        );
      }
    }
