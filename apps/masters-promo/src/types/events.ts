/**
 * Типы событий для коммуникации между хостом и плагином masters-promo
 *
 * Этот файл определяет типы данных для событий, которые используются
 * для обмена данными между хост-приложением и плагином masters-promo.
 * События разделены на две категории:
 * 1. События от хоста к плагину (Host -> Plugin)
 * 2. События от плагина к хосту (Plugin -> Host)
 */

import type { THostToPluginEvent, TPluginToHostEvent } from '@yclients-plugins/utils';

/**
 * Тип контекста плагина masters-promo
 ...
 */
export type THostReadyPayload = void;
