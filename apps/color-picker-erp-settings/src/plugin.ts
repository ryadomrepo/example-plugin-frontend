...
// Импортируем contract.json для получения PLUGIN_ID и PLUGIN_AREAS
import contract from '@/contract.json';

const PLUGIN_ID = contract['plugin-slug'];
const PLUGIN_AREAS = contract.packages['color-picker-erp-settings'].areas;
...
/**
 * Создает контекст плагина для color-picker-erp-settings
 * @param pluginId - Уникальный идентификатор плагина
 * @returns Объект контекста плагина
 */
const createPluginContext = (pluginId: string): TPluginContext => ({
  id: pluginId,
});
...
/**
 * Эмиттер событий для отправки сообщений от плагина к хосту
 * Используется для уведомления хоста о готовности плагина
 * @type {DocumentEventEmitter<TPluginReadyEvent>}
 */
